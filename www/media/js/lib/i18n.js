...
I18n.month_name = function(number) {
  number = parseInt(number, 10);
  var options = this.lookup("__date");
  if (!options) {
    return number;
  }
  return options.month_names[number];
};
...
I18n.missingTranslation = function() {
  ...
  for (var i = 0; i < count; i++) {
    message += "." + arguments[i];
    core_message += arguments[i];
  }

  message += '&quot; translation]';

  if(development_mode)
    $.post(url_base + 'translations/add_missing', {
      message: core_message.toLowerCase()
    });
...
I18n.capitalize = function(string) {
